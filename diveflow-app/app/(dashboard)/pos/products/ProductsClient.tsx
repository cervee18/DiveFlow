      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 text-sm bg-white border border-slate-200 rounded-lg focus:outline-none focus:border-teal-400 focus:ring-1 focus:ring-teal-400 placeholder-slate-400"
          />
          {search && (
            <button
              onClick={() => setSearch('')}
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button
            onClick={() => setSelectedCategoryId(selectedCategoryId === '__none__' ? '' : '__none__')}
            className={`px-3 py-2 rounded-lg text-xs font-semibold border transition-colors ${
              selectedCategoryId === '__none__'
                ? 'bg-slate-800 text-white border-slate-800'
        ...
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-64">
            ...
            <p className="text-xs text-slate-500 mt-1">
              {search || selectedCategoryId ? 'Try adjusting your search or filter.' : 'Get started by creating your first catalog item.'}
            </p>
          </div>
        ) : (
                  ...
                  <tr key={product.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-center">
                      {product.is_automated ? (
                        <span className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md text-xs font-semibold bg-indigo-50 text-indigo-700 border border-indigo-100" title="Automatically billed via Trip/Rental tracking">
                    ...
                    <td className="px-6 py-4 text-center">
                      {product.is_active ? (
                        <span className="inline-flex items-center w-2 h-2 rounded-full bg-emerald-500 ring-2 ring-emerald-100" title="Active" />
                      ) : (
                        <span className="inline-flex items-center w-2 h-2 rounded-full bg-slate-300 ring-2 ring-slate-100" title="Inactive" />
                      )}
                    </td>
