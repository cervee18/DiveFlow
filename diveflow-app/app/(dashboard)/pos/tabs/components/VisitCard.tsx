...
export default function VisitCard({ visit, selectedClientId, products, onSelectionChange, onAddItem, onWaiveItem, onDeleteItem, onEditItem, onEditAutoItem, onOpenTrip }: VisitCardProps) {
  const { payload } = visit;
                  ...
                  <div className="px-5 pb-3 space-y-2">
                    ...
                    {autoItems.map((item: any, idx: number) => (
                      <div key={`a-${idx}`} className={`flex justify-between items-center text-xs px-2.5 py-1.5 rounded-lg ${item.waived ? 'bg-slate-50 text-slate-400' : 'bg-slate-50 text-slate-500'}`}>
                        ...
                        <div className="flex items-center gap-1 shrink-0">
                          {item.trip_date && <span className="text-slate-400 mr-1">{formatDate(item.trip_date.split('T')[0])}</span>}
                          <span className={`font-mono mr-1 ${item.waived ? 'line-through text-slate-300' : ''}`}>{fmtMoney(item.price ?? 0)}</span>
                          {item.item_key && (
                            <>
                              {!item.waived && (
                                <button
                                  onClick={() => onEditAutoItem(visit.visitId, memberId, { itemKey: item.item_key, name: item.name, price: item.price ?? 0, basePrice: item.base_price ?? item.price ?? 0 })}
                                  title="Edit price"
                                  className="p-1 rounded transition-colors text-slate-300 hover:text-teal-600 hover:bg-teal-50"
                                >
                    ...
                    {manualItems.map((item: any, idx: number) => (
                      <div key={`m-${idx}`} className="flex justify-between items-center text-xs text-slate-500 px-2.5 py-1.5 bg-amber-50 rounded-lg border border-amber-100">
                        ...
                        <div className="flex items-center gap-1 shrink-0">
                          <span className="font-mono mr-1">{fmtMoney((item.price ?? 0) * (item.qty ?? 1))}</span>
                          {item.item_id && (
                            <>
                              <button
                                onClick={() => onEditItem({ id: item.item_id, name: item.name, price: item.price ?? 0, qty: item.qty ?? 1 })}
                                title="Edit item"
