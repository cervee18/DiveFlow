...
// FIX 3: Extract the page content into its own component so it can be wrapped
// in <Suspense> — required by Next.js whenever useSearchParams() is used in a
// Client Component (same pattern already used in clients/page.tsx).
function TripsContent() {
  const supabase = createClient();

  // FIX 3: Use useSearchParams() instead of window.location.search so that
  // Next.js routing is respected and the values are available on first render.
  ...
  // FIX 3: Read URL params through useSearchParams — runs on mount and
  // whenever the URL changes, not just once via a window.location snapshot.
  useEffect(() => {
        ...
        {selectedTripId && selectedTrip ? (
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 min-h-full p-8 flex flex-col">
            ...
            <TripManifest
              ...
              // Pass max_divers as the capacity — this is the operator-set cap,
              // independent of vessel capacity.
              tripCapacity={selectedTrip.max_divers}
...
// FIX 3: Wrap TripsContent in Suspense — Next.js requires this whenever
// useSearchParams() is called inside a Client Component.
export default function TripsPage() {
  return (
    <Suspense fallback={<div className="p-8 text-slate-500">Loading schedule...</div>}>
      <TripsContent />
    </Suspense>
  );
}