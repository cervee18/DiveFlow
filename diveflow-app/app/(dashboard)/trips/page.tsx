...
export default function TripsPage() {
  ...
  const [selectedDate, setSelectedDate] = useState(() => {
    ...
    return searchParams.get('date') ?? 
           (typeof window !== 'undefined' ? localStorage.getItem('diveflow_date') : null) ?? 
           todayStr;
  });
  ...
  // NEW: Keep state in sync if the user navigates here from another page 
  // with a different tripId or date in the URL
  useEffect(() => {
  ...
  // UPDATED: Keep URL in sync and share date with other pages via localStorage
  useEffect(() => {
    ...
    // Only replace the URL if our state requires a different URL than the current one
  ...
  }, [selectedDate, selectedTripId, router, searchParams]);
