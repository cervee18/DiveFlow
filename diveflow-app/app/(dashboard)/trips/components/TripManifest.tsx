                  ...
                  <tr key={diver.id} className={`${isModified ? 'bg-amber-50/40' : 'hover:bg-slate-50/50'} transition-colors group/row`}>
                    ...
                    <td className={`px-3 py-2 font-bold text-slate-900 border-r sticky left-0 z-10 shadow-[1px_0_0_0_#e2e8f0] ${isModified ? 'bg-amber-50' : 'bg-white'}`}>
                      <div className="flex items-center justify-between gap-2">
                        <span>{diver.clients?.first_name} {diver.clients?.last_name}</span>
                    ...
                    <td className="px-2 py-2 border-r text-center">
                      <input type="checkbox" checked={rowChanges.waiver ?? diver.waiver ?? false} onChange={e => handleChange(diver.id, 'waiver', e.target.checked)} className="rounded border-slate-300 text-blue-600 focus:ring-blue-500 cursor-pointer" />
                    </td>
                    <td className="px-2 py-2 border-r text-center">
                      <input type="checkbox" checked={rowChanges.deposit ?? diver.deposit ?? false} onChange={e => handleChange(diver.id, 'deposit', e.target.checked)} className="rounded border-slate-300 text-blue-600 focus:ring-blue-500 cursor-pointer" />
                    </td>
                    <td className="px-2 py-2 border-r text-center">
                      <input type="checkbox" checked={rowChanges.pick_up ?? diver.pick_up ?? false} onChange={e => handleChange(diver.id, 'pick_up', e.target.checked)} className="rounded border-slate-300 text-blue-600 focus:ring-blue-500 cursor-pointer" />
                    </td>

                    {/* Last Dive & Cert */}
                    <td className="px-3 py-2 border-r text-slate-500 font-medium">
                      {diver.clients?.last_dive_date ? formatLastDive(diver.clients.last_dive_date) : <span className="text-amber-600">New</span>}
                    </td>
                    <td className="px-3 py-2 border-r font-bold text-slate-700">
                      {diver.courses?.name ? <span className="text-blue-700 bg-blue-100 px-1.5 py-0.5 rounded text-[10px]">{diver.courses.name}</span> : (diver.clients?.certification_levels?.abbreviation || 'OW')}
                    </td>

                    {/* Equipment Dropdowns */}
                    {['bcd', 'wetsuit', 'fins', 'mask'].map(gear => (
                      <td key={gear} className="px-1 py-1 border-r bg-blue-50/10 hover:bg-white transition-colors">
                        <select value={rowChanges[gear] ?? diver[gear] ?? ''} onChange={e => handleChange(diver.id, gear, e.target.value)} className="w-full bg-transparent border-none focus:ring-1 focus:ring-blue-500 rounded text-[10px] font-bold text-slate-700 cursor-pointer appearance-none text-center">
                          <option value="">-</option>
                    ...
                    {/* Boolean Equipment */}
                    <td className="px-2 py-2 border-r text-center">
                      <input type="checkbox" checked={rowChanges.regulator ?? diver.regulator ?? false} onChange={e => handleChange(diver.id, 'regulator', e.target.checked)} className="rounded border-slate-300 text-blue-600 focus:ring-blue-500 cursor-pointer" />
                    </td>
                    <td className="px-2 py-2 border-r text-center">
                      <input type="checkbox" checked={rowChanges.computer ?? diver.computer ?? false} onChange={e => handleChange(diver.id, 'computer', e.target.checked)} className="rounded border-slate-300 text-blue-600 focus:ring-blue-500 cursor-pointer" />
                    </td>

                    {/* Gas Inline */}
                    <td className="px-2 py-1 border-r">
                      <div className="flex items-center gap-2 justify-center">
                         <input type="checkbox" checked={rowChanges.nitrox ?? diver.nitrox ?? false} onChange={e => handleChange(diver.id, 'nitrox', e.target.checked)} className="rounded border-slate-300 text-emerald-600 focus:ring-emerald-500 cursor-pointer"/>
                         {(rowChanges.nitrox ?? diver.nitrox) ? (
                           <input type="number" value={rowChanges.nitrox_percentage ?? diver.nitrox_percentage ?? 32} onChange={e => handleChange(diver.id, 'nitrox_percentage', parseInt(e.target.value))} className="w-10 text-[10px] font-black text-emerald-700 bg-emerald-50 border border-emerald-200 rounded p-0.5 text-center focus:ring-1 focus:ring-emerald-500"/>
