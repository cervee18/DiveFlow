...
export default function ClientProfileForm({
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const updates = {
      ...
      // FIX 1: cert_level is a UUID FK — save the id, not the name string.
      // Empty string (no cert selected) is coerced to null.
      cert_level: (formData.get("cert_level") as string) || null,
  ...
  // cert_level on the client row is already a UUID (or null).
  const currentCertLevelId: string = selectedClient.cert_level ?? "";

  // Resolve the full cert level object for the badge in the header.
  const currentCertLevel = certLevels.find((c) => c.id === currentCertLevelId);
      ...
      <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 shrink-0">
        <h2 className="text-lg font-semibold text-slate-800 flex items-center gap-5 truncate">
          ...
          {currentCertLevel && (
            <span
              title={
                selectedClient.nitrox_cert_number
                  ? `Nitrox Cert: ${selectedClient.nitrox_cert_number}`
                  : "Standard Air"
              }
      ...
      >
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none text-sm"
                required
              />
            </div>
            <div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            ...
            <div>
              ...
              {/* FIX 1: value is the cert level UUID, defaultValue matches the UUID stored on the client row */}
              <select
                name="cert_level"
                defaultValue={currentCertLevelId}
                className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none text-sm text-slate-700"
              >
                <option value="">No Certification Listed</option>
                <optgroup label="Recreational">
                  {certLevels
                    .filter((l) => !l.is_professional)
                    .map((l) => (
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              ...
              <input
                name="cert_number"
                defaultValue={selectedClient.cert_number || ""}
                className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none text-sm"
              />
            </div>
            <div>
              ...
              <input
                name="nitrox_cert_number"
                defaultValue={selectedClient.nitrox_cert_number || ""}
                className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none text-sm"
              />
            </div>
          </div>
          <div>
            ...
            <input
              name="last_dive_date"
              type="date"
              defaultValue={selectedClient.last_dive_date || ""}
              className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none text-sm"
            />
          </div>
        </div>
        ...
        <div>
          ...
          <div className="mb-4">
            ...
            <input
              name="address_street"
              defaultValue={selectedClient.address_street || ""}
              className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none text-sm"
            />
          </div>
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <input
                name="address_city"
                defaultValue={selectedClient.address_city || ""}
                className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none text-sm"
              />
            </div>
            <div>
              ...
              <input
                name="address_zip"
                defaultValue={selectedClient.address_zip || ""}
                className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none text-sm"
              />
            </div>
            <div>
              ...
              <input
                ...
                defaultValue={selectedClient.address_country || ""}
                className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none text-sm"
              />
            </div>
          </div>
        ...
        <div>
          ...
          <textarea
            ...
            rows={3}
            className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none text-sm"
            placeholder="Any special requirements or notes about this diver..."
          ></textarea>
        </div>
        ...
        <div className="pt-4 border-t border-slate-100 flex justify-end shrink-0 sticky bottom-0 bg-white">
          <button
            type="submit"
            disabled={isSaving}
            className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-md text-sm font-medium shadow-sm transition-colors disabled:opacity-70"
          >
