...
export default function DatePicker({ value, onChange }: DatePickerProps) {
  ...
  return (
    <div className="flex flex-col items-end gap-1 lg:flex-row lg:items-center">
      {/* Row 1: prev / date input / next — always horizontal */}
      ...
      {/* Row 2 on mobile / inline on desktop: Today shortcut */}
      {!isToday && (
        <button
          onClick={() => onChange(getTodayStr())}
          className="lg:ml-1 px-3 py-2 text-sm font-medium text-teal-600 hover:text-teal-700 border border-teal-200 hover:border-teal-300 rounded-lg bg-white transition-colors shadow-sm"
        >
          Today
        </button>
      )}
