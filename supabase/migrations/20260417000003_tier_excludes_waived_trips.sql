-- Fix: waived trips must not count toward volume tier thresholds.
-- Previously client_trip_type_counts included waived trips, inflating the
-- tier lookup and applying a bulk discount the client no longer qualifies for.
...
BEGIN
  ...
  -- Only count non-waived trips so tier thresholds reflect what is actually charged
  client_trip_type_counts AS (
    ...
    WHERE  tc.client_id IN (SELECT client_id FROM visit_clients_list)
      ...
      AND  NOT EXISTS (
             SELECT 1 FROM visit_waivers vw
             WHERE  vw.client_id = tc.client_id
               AND  vw.item_key  = 'trip:' || tc.id::text
           )
