-- ── Pricing rules: tiered trip prices + rental daily cap ─────────────────────
--
-- trip_pricing_tiers: volume discount tiers per trip type (retroactive).
--   When a client accumulates N trips of a type within a visit, ALL trips
--   of that type are repriced at the tier matching their total count.
--   e.g. 1+ trips → $75, 4+ trips → $65, 7+ trips → $55
--   If no tiers are defined, the flat price from pos_products is used.
--
-- organizations.rental_daily_cap: optional daily ceiling for rental gear.
--   Rental is now charged per (client × day), not per (client × trip).
--   If the sum of all rented gear items on a given day exceeds the cap,
--   the client is charged the cap amount for that day instead.

-- ── Schema changes ─────────────────────────────────────────────────────────

ALTER TABLE public.organizations
  ADD COLUMN IF NOT EXISTS rental_daily_cap numeric(10,2);

CREATE TABLE IF NOT EXISTS public.trip_pricing_tiers (
  id              uuid        PRIMARY KEY DEFAULT gen_random_uuid(),
  organization_id uuid        NOT NULL REFERENCES public.organizations(id) ON DELETE CASCADE,
  trip_type_id    uuid        NOT NULL REFERENCES public.trip_types(id)    ON DELETE CASCADE,
  min_qty         integer     NOT NULL CHECK (min_qty >= 1),
  unit_price      numeric(10,2) NOT NULL CHECK (unit_price >= 0),
  UNIQUE (trip_type_id, min_qty)
);
...
BEGIN
  ...
  -- ── Tank budget from courses added at the checkout terminal ───────────────
  ...
  -- ── Trip count per (client, trip_type) — basis for retroactive tier lookup ─
  client_trip_type_counts AS (
    ...
    WHERE  tc.client_id IN (SELECT client_id FROM visit_clients_list)
      ...
      AND  tc.activity_id IS NULL
      AND  tt.pos_product_id IS NOT NULL
    GROUP  BY tc.client_id, t.trip_type_id
  ),

  -- ── Resolve tier price: highest tier where min_qty <= trip_count ──────────
  --    NULL → no tier defined → fall back to flat pos_products.price
  ...
  -- ── Trips with window function for course-tank waiver + tier pricing ───────
  client_trips_windowed AS (
    SELECT tc.client_id,
           ...
           -- tier price overrides flat price when a matching tier exists
  ...
  --    GROUP BY deduplicates clients with multiple trips on the same day.
  ...
  -- ── All automated line items ──────────────────────────────────────────────
  automated_items AS (

    -- ① Trip charges: tier pricing + course-tank waiver (pro-rated at boundary)
    ...
    -- ③ Rental: per day, with optional cap label
    SELECT drt.client_id,
           jsonb_build_object(
             'name',      'Gear rental – ' || TO_CHAR(drt.trip_date, 'Mon DD')
                            || CASE WHEN drt.raw_total > drt.charged_amount
                               THEN ' (capped)' ELSE '' END,
    ...
    SELECT vcl.client_id,
           ...
           COALESCE(
             (SELECT jsonb_agg(item)
              FROM automated_items ai WHERE ai.client_id = vcl.client_id),
             '[]'::jsonb
           ) AS automated_items,
           COALESCE(
             (SELECT SUM(price_num)
              FROM automated_items ai WHERE ai.client_id = vcl.client_id),
             0
           ) AS auto_subtotal,
           COALESCE(
             (SELECT jsonb_agg(
              ...
              WHERE  pii.invoice_id = v_invoice_id
                AND  pii.client_id  = vcl.client_id),
             '[]'::jsonb
           ) AS manual_items,
           COALESCE(
           ...
           COALESCE(
             (SELECT jsonb_agg(
                       jsonb_build_object(
                         'date',   ppay.created_at,
                         'amount', ppay.amount,
                         'method', ppay.payment_method
                       ))
