...
BEGIN
  ...
  -- Trips belong to a visit when the trip's start_time falls within the visit's date range
  -- and the client appears in both visit_clients and trip_clients.
  ...
  automated_trip_items AS (
    ...
    SELECT
      tc.client_id,
      jsonb_build_object('name', pp.name, 'price', pp.price, 'type', 'trip') AS item,
    ...
    SELECT
      tc.client_id,
      jsonb_build_object('name', pp.name, 'price', pp.price, 'type', 'activity') AS item,
      pp.price AS price_num
    FROM public.trip_clients tc
    JOIN public.trips t ON t.id = tc.trip_id
    ...
    SELECT
      tc.client_id,
      jsonb_build_object('name', pp.name, 'price', pp.price, 'type', 'course') AS item,
      pp.price AS price_num
    FROM public.trip_clients tc
    JOIN public.trips t ON t.id = tc.trip_id
    ...
    SELECT
      tc.client_id,
      jsonb_build_object('name', pp.name, 'price', pp.price, 'type', 'rental') AS item,
      pp.price AS price_num
    FROM public.trip_clients tc
    JOIN public.trips t ON t.id = tc.trip_id
  ...
  -- Shared group items (invoice items with no client assigned)
  SELECT COALESCE(
