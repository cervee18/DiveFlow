-- Fix generate_trips_from_schedule: switch from bulk INSERT to row-by-row loop
-- so vessel_overlap exceptions on individual slots are caught and skipped rather
...
BEGIN
  ...
  FOR v_row IN
    SELECT
      slot.vessel_id,
      slot.trip_type_id,
      ((d.date_val + slot.start_time)::timestamp AT TIME ZONE v_timezone) AS trip_start,
      COALESCE(
  ...
  LOOP
    ...
    EXCEPTION WHEN OTHERS THEN
      -- Skip this slot (vessel overlap or other constraint violation)
      NULL;
    END;
  END LOOP;
