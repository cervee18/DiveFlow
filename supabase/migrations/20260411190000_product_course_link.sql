-- Link products to courses (many products → one course).
-- Replaces the old one-to-one courses.pos_product_id join in the RPC.

ALTER TABLE public.pos_products
  ADD COLUMN IF NOT EXISTS course_id uuid REFERENCES public.courses(id) ON DELETE SET NULL;

-- Migrate existing data: if a course already pointed at a product, set the reverse link.
UPDATE public.pos_products pp
SET    course_id = c.id
FROM   public.courses c
WHERE  c.pos_product_id = pp.id
  AND  pp.course_id IS NULL;

-- ── RPC: waive trips via pos_products.course_id instead of courses.pos_product_id ──
...
BEGIN
  ...
  -- Looks up pos_products.course_id so multiple products can map to one course.
  client_included_trips AS (
    ...
    JOIN   public.pos_products      prod ON prod.id   = pii.pos_product_id
    JOIN   public.courses           c    ON c.id      = prod.course_id
