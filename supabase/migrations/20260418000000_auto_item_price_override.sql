-- Price overrides for automated POS line items (trips, rentals, guide fees).
-- Mirrors the pos_auto_item_waivers pattern: keyed by (visit_id, client_id, item_key).
-- The RPC reads overrides at calculation time and applies them before waivers.

CREATE TABLE public.pos_auto_item_price_overrides (
...
DO $$ BEGIN
  CREATE POLICY "org members can manage auto item price overrides"
    ON public.pos_auto_item_price_overrides
    FOR ALL
...
-- Rebuild RPC: add visit_price_overrides CTE and apply it in automated_items.
-- Also adds 'base_price' to each auto item JSON so the UI can show the original price.
...
BEGIN
  ...
  automated_items AS (
    ...
    LEFT   JOIN visit_price_overrides po
           ON  po.client_id = ctl.client_id
           AND po.item_key  = ctl.item_key
    ...
    LEFT   JOIN visit_price_overrides po
           ON  po.client_id = tc.client_id
           AND po.item_key  = 'guide:' || t.id::text
    WHERE  tc.client_id IN (SELECT client_id FROM visit_clients_list)
      AND  t.start_time::date BETWEEN v_visit_start AND v_visit_end
      AND  tc.private = true
    ...
    LEFT   JOIN visit_price_overrides po
           ON  po.client_id = dri.client_id
           AND po.item_key  = 'rental:' || dri.trip_date::text
    ...
    LEFT   JOIN visit_price_overrides po
           ON  po.client_id = drt.client_id
           AND po.item_key  = 'rental:' || drt.trip_date::text
    WHERE  drt.raw_total > drt.charged_amount
  ),
