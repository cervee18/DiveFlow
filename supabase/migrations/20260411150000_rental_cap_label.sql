-- Rename capped rental label from 'Gear rental – Mon DD (capped)' to 'Full Rental Gear'
...
BEGIN
  ...
  automated_items AS (
    ...
    SELECT drt.client_id,
           jsonb_build_object(
             'name',      CASE WHEN drt.raw_total > drt.charged_amount
                               THEN 'Full Rental Gear'
                               ELSE 'Gear rental – ' || TO_CHAR(drt.trip_date, 'Mon DD')
                          END,
