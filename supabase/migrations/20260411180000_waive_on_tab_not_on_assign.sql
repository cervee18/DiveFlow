-- Fix: course trips were silently excluded from billing the moment an activity
-- was assigned to a client (tc.activity_id IS NULL filter).
-- Now they appear as normal chargeable trips and are only waived once the
-- course product is explicitly added to the client's tab (pos_invoice_items).
...
BEGIN
  ...
  -- activity_id is no longer used to exclude trips; exclusion happens via
  -- billing_via_activity = false (trip types billed through an activity
  -- programme are still excluded as before).
