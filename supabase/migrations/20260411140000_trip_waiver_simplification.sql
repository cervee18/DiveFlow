-- Simplify course waiver: count trips instead of tanks
-- Removes tanks_count from trip_types (no longer needed).
-- Renames included_dives → included_trips on courses.
-- The RPC now waives whole trips chronologically until the course allowance
...
BEGIN
  ...
  -- ── Trips included by courses added at the checkout terminal ─────────────
  ...
  -- ── Trips ordered chronologically for course-trip waiver ─────────────────
  ...
  automated_items AS (
    ...
    -- ① Trip charges: skip the first N trips covered by the course allowance
    ...
    -- ③ Rental: per day, with optional cap
